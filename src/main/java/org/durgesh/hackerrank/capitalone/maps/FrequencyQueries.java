 ...
 *      tc #8,9,12 are failing (wrong answer)
 *      tc #10,11 (28 secs) are timed out. the answer is also wrong for tc #11.
 * TODO: when withRevMap = true
 *      tc #5,8,9,10,12 are failing (wrong answer)
...
public class FrequencyQueries {
    ...
    static void updateRevMap(int oldFreq, int newFreq) {
        ...
        if (revMap.containsKey(oldFreq)) {//old freq
    ...
    static void deleteOne(int n, Map<Integer, Integer> freqMap) {
        int freq = freqMap.getOrDefault(n, 0);
        int newFreq = freq-1;
        freqMap.put(n, newFreq);
        updateRevMap(freq, newFreq);
    }
...
//        runSampleTCs();
    ...
    static void write2File(List<Integer> ans) {
        try(BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter("/Users/durgeshverma/IdeaProjects/leetcode/src/main/resources/frequeries/ro11-2.txt"))) {
