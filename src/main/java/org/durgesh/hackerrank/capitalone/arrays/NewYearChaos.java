...
public class NewYearChaos {
    ...
    static void minimumBribesGood2(int[] q) {
        int c = 0;
        for (int i = q.length-1; i >=0; i--) {
            int rightPos = i+1;
            if (q[i] != rightPos) {//if current element is not at rightPos
                if (q[i-1]==rightPos) {//if prev element is at rightPos, then count n swap
                    c++;
                    swap(q, i, i-1);
                } else if (q[i-2]==rightPos) {//if prev to prev element is at rightPos, then count 2 n swap twice
                    c += 2;
                    swap(q, i-2, i-1);
                    swap(q, i-1, i);
                } else {
    ...
    static void minimumBribesGood2x(int[] q) {
        int c = 0;
    ...
    static void minimumBribesBad(int[] q) {
        ...
        for (int i = 0; i < q.length; i++) {
            int initPos = q[i];
            int currPos = i + 1;
            int b = initPos - currPos;
            if (b > 2) {
                System.out.println("chaotic! initPos="+initPos+ ";currPos="+ currPos);
                chaotic = true;
                break;
            } else if (b > 0 && b <= 2) {
                c += b;
            } else {
                if (b < 0) negatives.add(i);
                continue;
            }
        }
//        System.out.println(negatives);
        int max = q[negatives.get(0)];
        for (int i = 1; i < negatives.size(); i++) {
            int temp = q[negatives.get(i)];
            if (max > temp) {//out of order - record swap
                c++;
            } else {
                max = temp;
            }
    ...
    static void minimumBribesGood(int[] q) {
        //1, 2, 5, 3, 7, 8, 9, 10, 11, 12, 6, 4
        int c = 0;
        boolean chaotic = false;
        for (int i = 0; i < q.length; i++) {
            int initPos = q[i];
            int currPos = i + 1;
            if (initPos - currPos > 2) {
                System.out.println("chaotic! initPos="+initPos+ ";currPos="+ currPos);
                chaotic = true;
                break;
            } else {
...
//        int[] q = new int[] {
//                2,1,5,6,3,4,9,8,11,7,10,14,13,12,17,16,15,19,18,22,20,24,23,21,27,28,25,26,30,
//                29,33,32,31,35,36,34,39,38,37,42,40,44,41,43,47,46,48,45,50,52,49,51,54,56,55,
//                53,59,58,57,61,63,60,65,64,67,68,62,69,66,72,70,74,73,71,77,75,79,78,81,82,80,
//                76,85,84,83,86,89,90,88,87,92,91,95,94,93,98,97,100,96,102,99,104,101,105,103,
//                108,106,109,107,112,111,110,113,116,114,118,119,117,115,122,121,120,124,123,127,
//                125,126,130,129,128,131,133,135,136,132,134,139,140,138,137,143,141,144,146,145,142,
//                148,150,147,149,153,152,155,151,157,154,158,159,156,161,160,164,165,163,167,166,162,
//                170,171,172,168,169,175,173,174,177,176,180,181,178,179,183,182,184,187,188,185,190,189,
//                186,191,194,192,196,197,195,199,193,198,202,200,204,205,203,207,206,201,210,209,211,208,
//                214,215,216,212,218,217,220,213,222,219,224,221,223,227,226,225,230,231,229,228,234,235,233,
//                237,232,239,236,241,238,240,243,242,246,245,248,249,250,247,244,253,252,251,256,255,258,254,
//                257,259,261,262,263,265,264,260,268,266,267,271,270,273,269,274,272,275,278,276,279,277,282,
//                283,280,281,286,284,288,287,290,289,285,293,291,292,296,294,298,297,299,295,302,301,304,303,
//                306,300,305,309,308,307,312,311,314,315,313,310,316,319,318,321,320,317,324,325,322,323,328,
//                327,330,326,332,331,329,335,334,333,336,338,337,341,340,339,344,343,342,347,345,349,346,351,
